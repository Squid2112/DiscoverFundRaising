component output="no" {
	...
	this.Name = "DiscoverFundraising";
	this.applicationName = "DiscoverFundraising";
...
// ---------------------< onApplicationStart >---------------------
	public boolean function onApplicationStart() {
		...
		Application.Settings = {
			...
			DSN = { System="FundRaising" },
			rootDomain = "discoverfundraising.com",
			defaultConical = "www",
			...
			forceMobile = true,
			abuseRedirect = "",
			processExceptions = ["/com/","/css/","/js/","/assets/","favicon.ico","robots.txt"],
		...
		if(Application.Settings.cacheCom) Application.System.loadCacheObjects(path="com",clear=true);
		...
		Application.cachedCfcs = structNew();
	...
	public void function onSessionStart() {
		...
		try {
			if(Application.Settings.cacheCom) {
				structInsert(Session,"Visitor",createObject("component","inram." & this.applicationName & ".com.Visitor.VisitorObject").init(),true);
			} else {
				structInsert(Session,"Visitor",createObject("component","com.Visitor.VisitorObject").init(),true);
			}
		} catch(Any E) {
			saveContent variable="result" { writeDump(E); }
			fileWrite(Application.Settings.rootPath & "exception.html",e,"utf-8");
			mailService.setTo("hostmaster@thixo.net");
			mailService.setFrom("hostmaster@thixo.net");
			mailService.setSubject("Application.System.fillCache() error in Application.cfc");
			mailService.setType("html");
...
// ---------------------< onRequestStart >---------------------
	public function onRequestStart(required string requestPage) {
		...
		if(!(find("108.214.161.",Session.Visitor.requestData.IPaddress) || find("76.22.",Session.Visitor.requestData.IPaddress))) {
			location("http://www.discoverindy.com/fundraising-with-the-discover-indy",false,"301");
		}
	...
	public void function onRequest(required string requestPage) {
		if(!Request.isException && !Request.isCFC) {
			if(fileExists(Application.Settings.rootPath & "views\" & Request.virtualInfo.viewName & "\" & Request.virtualInfo.viewTemplate)) {
				try {
					Application.System.writeBaseDocument(Request=Request,virtualInfo=Request.virtualInfo);
				} catch(Any e) {
				...
				try {
//					Application.System.Wrappers.include("/views/" & Request.virtualInfo.viewName & "/" & Request.virtualInfo.viewTemplate);
...
// ---------------------< onCfcRequest >---------------------
	public any function onCfcRequest(required string cfcname, required string method, required struct args) output="yes" {
		if(!structKeyExists(Application.cachedCfcs,Arguments.cfcname)) {
			try {
				Application.cachedCfcs[Arguments.cfcname] = createObject(Arguments.cfcname);
			} catch(Any e) {
				writeDump(e);
				return;
			}
		...
		try {
			var obj = Application.cachedCfcs[Arguments.cfcname];
			var func = obj[Arguments.method];
			var result = func(argumentCollection=Arguments.args);
		} catch(Any e) {
			writeDump(e);
			return;
		}
...
// ---------------------< onMissingTemplate >---------------------
	public boolean function onMissingTemplate(required string requestPage) {
		...
		if(structKeyExists(Application,"Database")) {
			var virtualMappingData = Application.Database.getVirtualMapping(cgi.SCRIPT_NAME);
			if(isDefined("virtualMappingData") && virtualMappingData.recordCount && structCount(url)) {
				location(virtualMappingData.dest[1] & "?" & cgi.QUERY_STRING,false,"301");
			} else if(isDefined("virtualMappingData") && virtualMappingData.recordCount) {
...
//		if((arrayLen(structFindKey(Session,"showErrors")) && Session.Visitor.showErrors) || !arrayLen(structFindKey(Session,"showErrors"))) {
...
// ---------------------< showDebug >---------------------
	public void function showDebug() output="yes" {
		writeOutput('<div style="position:absolute;top:1px;left:1px;display:block;float:left;">');

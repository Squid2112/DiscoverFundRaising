component displayname="CoreSystemObjectComponent" hint="Core System Object Component" output="false" {
	...
	public any function init(string dsn=Application.Settings.DSN.System) output="false" {
		...
		this.dsnValid = this.verifyDSN(this.DSN);
		this.urlClass = createObject("java","java.net.URL");
		...
		this.Frameworks = structNew();
		if(fileExists(Application.Settings.rootPath & 'com\Manifests\Frameworks.manifest.json')) {
			manifest = fileRead(Application.Settings.rootPath & 'com\Manifests\Frameworks.manifest.json');
			this.Frameworks = duplicate(this.jsonDecode(manifest));
			if(!structKeyExists(this,'Frameworks')) this.Frameworks = structNew();
		}
		for(item IN this.Frameworks) {
			this.Frameworks[item].coreFramework = { start = '', end = '' };
			this.Frameworks[item].baseFramework = { start = '', end = '' };
			this.Frameworks[item].cached = false;
		}
		if(Application.Settings.cache) {
			for(item IN this.Frameworks) {
				if(this.Frameworks[item].cacheable) {
					if(fileExists(Application.Settings.rootPath & 'Frameworks\' & item & '\coreFrameworkStart.cfm')) this.Frameworks[item].coreFramework.start = fileRead(Application.Settings.rootPath & 'Frameworks\' & item & '\coreFrameworkStart.cfm');
					if(fileExists(Application.Settings.rootPath & 'Frameworks\' & item & '\coreFrameworkEnd.cfm')) this.Frameworks[item].coreFramework.end = fileRead(Application.Settings.rootPath & 'Frameworks\' & item & '\coreFrameworkEnd.cfm');
					if(fileExists(Application.Settings.rootPath & 'Frameworks\' & item & '\baseFrameworkStart.cfm')) this.Frameworks[item].baseFramework.start = fileRead(Application.Settings.rootPath & 'Frameworks\' & item & '\baseFrameworkStart.cfm');
					if(fileExists(Application.Settings.rootPath & 'Frameworks\' & item & '\baseFrameworkEnd.cfm')) this.Frameworks[item].baseFramework.end = fileRead(Application.Settings.rootPath & 'Frameworks\' & item & '\baseFrameworkEnd.cfm');
					this.Frameworks[item].cached = true;
				}
			}
		}
...
*/
	...
	public struct function virtualResolver(required string vPath) output="false" {
		Arguments.vPath = trim(Arguments.vPath);
		var	vInfo = {
				recordCount = 0,
				URI = Arguments.vPath,
				URL = '',
		...
		if(structKeyExists(Application,'Database')) {
			var qVU = Application.Database.getVirtualUri(listLast(Arguments.vPath,'/'));
			if(qVU.metaInfo.recordCount) {
				vInfo = {
					...
					virtualPath = Arguments.vPath,
					virtualUriId = qVU.result.virtualUriId[1],
		...
		if(!vInfo.recordCount) {
			if(fileExists(expandPath('/views' & Arguments.vPath) & '\index.cfm')) {
				vInfo.ViewName = listLast(Arguments.vPath,'/');
			} else {
				if(fileExists(expandPath('/') & 'Views' & cgi.SCRIPT_NAME)) {
					vInfo.contentType = listLast(cgi.SCRIPT_NAME,'.');
					vInfo.recordCount = 1;
					vInfo.URI = replace(cgi.SCRIPT_NAME,listLast(cgi.SCRIPT_NAME,'/'),'');
					vInfo.viewName = replace(cgi.SCRIPT_NAME,listLast(cgi.SCRIPT_NAME,'/'),'');
					vInfo.viewTemplate = listLast(cgi.SCRIPT_NAME,'/');
					vInfo.virtualPath = replace(cgi.SCRIPT_NAME,listLast(cgi.SCRIPT_NAME,'/'),'');
					vInfo.virtualUriId = 0;
...
//		Arguments.Visitor.Search.setSearchFromUrl(vSearchUrl=duplicate(sets),vSortOptions=duplicate(sortOptions));
	...
	public void function writeBaseDocument(required struct Request, required struct virtualInfo) output="true" {
		var s = createObject("java","java.lang.StringBuffer").init("");
		var viewTemplateName = listFirst(Arguments.virtualInfo.viewTemplate,".");
		var i = 0;

		if(Application.Settings.cache && structKeyExists(this.Frameworks,Arguments.Request.virtualInfo.Framework) && this.Frameworks[Arguments.Request.virtualInfo.Framework].cached) {
			writeOutput(this.Frameworks[Arguments.Request.virtualInfo.Framework].coreFramework.start);
		} else {
			if(fileExists(Application.Settings.rootPath & 'Frameworks\' & Arguments.Request.virtualInfo.Framework & '\coreFrameworkStart.cfm')) this.Wrappers.include('/Frameworks/' & Arguments.Request.virtualInfo.Framework & '/coreFrameworkStart.cfm');
		}
...
*/
		...
		if(Application.Settings.cache && structKeyExists(this.Frameworks,Arguments.Request.virtualInfo.Framework) && this.Frameworks[Arguments.Request.virtualInfo.Framework].cached) {
			writeOutput(this.Frameworks[Arguments.Request.virtualInfo.Framework].baseFramework.start);
		} else {
			if(fileExists(Application.Settings.rootPath & "Frameworks\" & Arguments.Request.virtualInfo.Framework & '\baseFrameworkStart.cfm')) this.Wrappers.include('/Frameworks/' & Arguments.Request.virtualInfo.Framework & '/baseFrameworkStart.cfm');
		}
	...
	public void function closePageContents(required struct Request) output="true" {
		...
		if(Application.Settings.cache && structKeyExists(this.Frameworks,Arguments.Request.virtualInfo.Framework) && this.Frameworks[Arguments.Request.virtualInfo.Framework].cached) {
			writeOutput(this.Frameworks[Arguments.Request.virtualInfo.Framework].baseFramework.end);
		} else {
			if(fileExists(Application.Settings.rootPath & 'Frameworks\' & Arguments.Request.virtualInfo.Framework & '\baseFrameworkEnd.cfm')) this.Wrappers.include('/Frameworks/' & Arguments.Request.virtualInfo.Framework & '/baseFrameworkEnd.cfm');
		}

		if(Application.Settings.cache && structKeyExists(this.Frameworks,Arguments.Request.virtualInfo.Framework) && this.Frameworks[Arguments.Request.virtualInfo.Framework].cached) {
			writeOutput(this.Frameworks[Arguments.Request.virtualInfo.Framework].coreFramework.end);
		} else {
			if(fileExists(Application.Settings.rootPath & 'Frameworks\' & Arguments.Request.virtualInfo.Framework & '\coreFrameworkEnd.cfm')) this.Wrappers.include('/Frameworks/' & Arguments.Request.virtualInfo.Framework & '/coreFrameworkEnd.cfm');
		}

		if(structKeyExists(Arguments.Request,"cssQueue")) {
			while(!Arguments.Request.cssQueue.empty()) {
				filename = Arguments.Request.cssQueue.pop();
				compressIt = !(left(filename,1) == "!");
				if(!compressIt) filename = right(filename,len(filename)-1);
				if(fileExists(Application.Settings.rootPath & filename)) {
					content = fileRead(Application.Settings.rootPath & filename);
					if(Application.Settings.compressCss && compressIt) {
						content = this.cssCompressor(content);
						if(len(content)) this.Wrappers.htmlHead(text='<style type="text/css">' & content & '</style>');
					} else {
						this.Wrappers.htmlHead(text='<script type="text/javascript" src="/' & filename & '"></script>');
					}
				}
			}
		}
		
		if(structKeyExists(Arguments.Request,"jsQueue")) {
			while(!Arguments.Request.jsQueue.empty()) {
				...
				if(fileExists(Application.Settings.rootPath & filename)) {
					content = fileRead(Application.Settings.rootPath & filename);
					if(Application.Settings.compressJs && compressIt) {
						content = this.jsCompressor(content);
						if(len(content)) this.Wrappers.htmlHead(text='<script type="text/javascript">' & content & '</script>');
					} else {
						this.Wrappers.htmlHead(text='<script type="text/javascript" src="/' & filename & '"></script>');
					}
	...
	public void function processView(required struct virtualInfo) output="true" {
		...
		try {
			...
			Application.Mail.send(to=Application.Settings.emailLists.Errors,subject='[#cgi.SERVER_NAME#] Application Error',msg='There was an Application error at #cgi.SERVER_NAME#',obj=debug);
		}
	}
	...
	public void function loadFrameworkObjects(required string scope) output="false" {
		...
		for(i=1; i <= arrayLen(this.Objects[Arguments.scope]); i++) {
			if(!Application.Settings.cacheCom) {
				if(this.Objects[Arguments.scope][i].init) {
					ptrScope[this.Objects[Arguments.scope][i].name] = createObject(this.Objects[Arguments.scope][i].type,'inram.' & Application.applicationName & '.' & this.Objects[Arguments.scope][i].class).init();
				} else {
					ptrScope[this.Objects[Arguments.scope][i].name] = createObject(this.Objects[Arguments.scope][i].type,'inram.' & Application.applicationName & '.' & this.Objects[Arguments.scope][i].class);
				}
			} else {
				if(this.Objects[Arguments.scope][i].init) {
...
*/
	...
	public any function loadCacheObjects(required string path, boolean clear=false) output="false" {
		var z = 0;
		var slimDir = "";
		var tmpFile = "";
		var qDir = "";
		...
		try {
			if(Arguments.clear) directoryDelete('ram:///' & Application.applicationName & '/' & Arguments.path, true);
		} catch(any e) { }

		qDir = directoryList(Application.Settings.rootPath & path,true,'query');
		for(z=1; z <= qDir.recordCount; z++) {
			slimDir = replaceNoCase(qDir.Directory[z],Application.Settings.rootPath,'') & '\' & qDir.Name[z];
			if(qDir.type[z] == 'Dir') {
				if(!directoryExists('ram:///' & Application.applicationName & '\' & slimDir)) directoryCreate('ram:///' & Application.applicationName & '\' & slimDir);
			} else if(qDir.type[z] == 'File') {
				tmpFile = fileReadBinary(qDir.Directory[z] & '\' & qDir.Name[z]);
				fileWrite('ram:///' & Application.applicationName & '\' & slimDir,tmpFile);
			}
		}
	}
	...
	public boolean function loadIntoRam(required string filePath) output="false" {
		...
		var dirPath = listDeleteAt(listRest(Arguments.filePath,'\'),listLen(listRest(Arguments.filePath,'\'),'\'),'\');

		if(!directoryExists('ram:///' & dirPath)) directoryCreate('ram:///' & dirPath);

		if(fileExists(Arguments.filePath)) {
			try {
				tmpFile = fileRead(Arguments.filePath);
				fileWrite('ram:///' & dirPath & '\' & fileName,tmpFile);
			} catch(Any E) {
	...
	public void function css(required struct Request, required string filename) output="false" {
		if(!structKeyExists(Arguments.Request,'cssQueue') || (Arguments.Request.cssQueue.search(Arguments.filename) != -1)) return;
	...
	public void function js(required struct Request, required string filename, string params) output="false" {
		if(!structKeyExists(Arguments.Request,"jsQueue") || (Arguments.Request.jsQueue.search(Arguments.filename) != -1)) return;
	...
	public boolean function isTrueNumeric(required string str) output="false" {
	...
	public boolean function verifyDSN(required string dsn) output="false" {
		try {
			var factory = createObject('java','coldfusion.server.ServiceFactory');
			var dsService = factory.getDataSourceService();
